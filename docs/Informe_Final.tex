...
\begin{abstract}
	...
	Experimental evaluation on synthetic instances ranging from 10 to 150 freelancers demonstrates that the hybrid algorithm achieves optimal or near-optimal solutions with exceptional efficiency, solving the largest instances in under 0.001 seconds. Comparative analysis reveals that while greedy algorithms offer minimal execution time, they exhibit suboptimal performance in complex scenarios, whereas genetic algorithms provide consistent quality at higher computational cost.
...
\subsubsection{Complexity Analysis}
The time complexity of the greedy approximation algorithm is determined by its nested loop structure. The main \texttt{while} loop iterates until all required skills are covered. In the worst-case scenario, where each selected freelancer covers only one new skill, this loop executes $O(n)$ times, where $n = |R|$ is the number of required skills.

Within each iteration of the \texttt{while} loop, the algorithm must find the best freelancer. This is done by iterating through all remaining available freelancers (at most $m = |F|$) in the \texttt{for} loop. For each of these freelancers, it calculates the set of newly covered skills, which requires checking against the currently uncovered skills. This check takes $O(n)$ time. Therefore, finding the best freelancer in each iteration has a complexity of $O(m \cdot n)$.

Combining these factors, the total time complexity is the product of the \texttt{while} loop iterations and the work done per iteration: $O(n) \times O(m \cdot n) = O(m \cdot n^2)$. The space complexity is $O(m+n)$ to store the freelancer data, skill requirements, and the sets of covered skills.
...
The time complexity of the local search algorithm is dependent on the number of improvements found, denoted by $k$. The main \texttt{while} loop runs $k$ times, and in each iteration, it explores three types of neighborhoods:

\begin{enumerate}
    \item \textbf{2-for-1 Swap Operator:} This is the most computationally expensive operator. It considers every pair of freelancers in the solution ($O(m^2)$ pairs), and for each pair, it iterates through every freelancer not in the solution ($O(m)$). The coverage check for each combination takes $O(n)$ time. Thus, the strict worst-case complexity for this part is $O(m^2 \cdot m \cdot n) = O(m^3 \cdot n)$.
    \item \textbf{Redundancy Elimination:} This operator iterates through each of the (at most) $m$ freelancers in the solution and performs a full coverage check with the remaining team, which takes $O(m \cdot n)$. The total complexity for this part is $O(m \cdot m \cdot n) = O(m^2 \cdot n)$.
    \item \textbf{1-for-1 Swap Operator:} This operator considers every freelancer in the solution ($O(m)$) and every freelancer outside the solution ($O(m)$), performing a full coverage check ($O(m \cdot n)$) for each valid swap. The complexity is $O(m \cdot m \cdot m \cdot n) = O(m^3 \cdot n)$.
\end{enumerate}

The overall complexity of a single iteration is dominated by the swap operators, resulting in $O(m^3 \cdot n)$. Since the loop runs $k$ times, the total time complexity of the algorithm as implemented is $O(k \cdot m^3 \cdot n)$. This is a more accurate reflection of the algorithm's performance than the simplified quadratic estimate.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Comment{Operator 1: 2-for-1 Swap}
		\For{\textbf{each} pair $(f_1, f_2) \in \text{current\_solution}$} ... \textbf{break} \EndFor
		\If{$\text{improved}$} \textbf{continue} \EndIf

		\Comment{Operator 2: Redundancy Elimination}
		\For{\textbf{each} $f \in \text{current\_solution}$} ... \textbf{break} \EndFor
		\If{$\text{improved}$} \textbf{continue} \EndIf

		\Comment{Operator 3: 1-for-1 Swap}
		\For{\textbf{each} $f_{in} \in \text{current\_solution}$}
		\For{\textbf{each} $f_{out} \in F \setminus \text{current\_solution}$}
		\If{$f_{out}.\text{cost} < f_{in}.\text{cost}$}
		\State $\text{candidate} \gets (\text{current\_solution} \setminus \{f_{in}\}) \cup \{f_{out}\}$
		\If{$\text{candidate}$ covers $R$}
		\State $\text{current\_solution} \gets \text{candidate}$
		\State $\text{improved} \gets \text{true}$; \textbf{break}
		\EndIf
		\EndIf
		\EndFor
		\If{$\text{improved}$} \textbf{break} \EndIf
		\EndFor
		\EndWhile
...
\paragraph{Complexity Analysis}
The time complexity of the genetic algorithm is determined by the population size $P$, the number of generations $G$, and the cost of evaluating the fitness of each individual. The main loop of the algorithm runs for $G$ generations.

Within each generation, the most computationally intensive step is the fitness evaluation for each of the $P$ individuals in the population. The fitness function must verify if a solution is valid (i.e., covers all $n$ required skills) and calculate its total cost. The validity check requires iterating through each of the $n$ skills and, in the worst case, checking against all freelancers in the solution (at most $m$). This results in a complexity of $O(m \cdot n)$ for a single evaluation.

Therefore, the total work per generation is $O(P \cdot m \cdot n)$. Multiplying by the number of generations $G$, the overall time complexity of the genetic algorithm is $O(G \cdot P \cdot m \cdot n)$.

\subsection{Kernelization Techniques}
\label{subsec:kernelization}

Kernelization techniques were implemented to reduce the size of the problem instance while preserving the optimal solution. These preprocessing steps help improve the efficiency of both exact and heuristic algorithms by eliminating redundant or dominated freelancers and simplifying the skill coverage requirements.

The first technique involves \textbf{removing dominated freelancers}. A freelancer $f_1$ is considered dominated by another freelancer $f_2$ if $f_2$ is strictly better in all aspects: $f_2$ covers all skills that $f_1$ covers (and possibly more) at a lower or equal cost. Formally, if $S(f_1) \subseteq S(f_2)$ and $c(f_1) \geq c(f_2)$, then $f_1$ can be safely removed from consideration. This reduction can be performed in $O(m^2 \cdot n)$ time by comparing all pairs of freelancers.

The second technique focuses on \textbf{identifying essential skills}. If a skill is only covered by a single freelancer, that freelancer must be included in any valid solution. These essential freelancers are selected first, and their covered skills are removed from the remaining requirements. This process is repeated until no more essential freelancers can be found. This step runs in $O(m \cdot n)$ time per iteration and typically requires only a few iterations to converge.

These kernelization techniques can significantly reduce the problem size in practice, especially for instances where many freelancers have overlapping skill sets or when certain skills are rare in the workforce. The reduced instance can then be solved more efficiently by the main algorithms, with the solution to the original problem obtained by combining the essential freelancers with the solution to the reduced instance.
...
\begin{table}[ht]
	...
	\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{3.5cm}>{\raggedright\arraybackslash}p{5cm}@{}}
		...
		Greedy Approximation & $O(m\cdot n^2)$ & $O(\log n)$-approximation & Fast decent solutions \\ \addlinespace
		Local Search & $O(k \cdot m^3 \cdot n)$ & Improves initial solution & Post-processing refinement \\ \addlinespace
...
The hybrid approach's exceptional performance derives from its synergistic combination of preprocessing and algorithmic adaptation. Kernelization rules effectively exploit problem structure to eliminate dominated candidates while preserving solution optimality. The observed 92\% reduction rate for $m=150$ indicates that real-world MCSC instances may contain substantial redundancy, presenting significant opportunities for computational acceleration through domain-specific analysis.

\subsection{Conclusion of Experimental Analysis}
\label{subsec:experimental-conclusion}

Empirical evaluation validates the hybrid algorithm as the most effective approach for the MCSC problem, successfully balancing solution quality with computational efficiency. The methodology demonstrates that problem-specific preprocessing through kernelization can dramatically reduce search space complexity, enabling exact solutions on substantially reduced instances. This approach achieves optimality guarantees where pure exact methods prove computationally prohibitive while maintaining execution speeds comparable to simple heuristics. These findings emphasize the critical importance of domain knowledge and preprocessing in addressing NP-hard optimization problems, suggesting that similar hybrid strategies may prove effective for related combinatorial optimization challenges.
...
Practical deployment considerations emerge from the efficiency-quality trade-offs observed. The hybrid approach provides optimal solutions with negligible execution time (0.0006 seconds for $m=150$), making it suitable for real-time applications. The genetic algorithm offers reliable performance as a fallback mechanism, particularly for instances where kernelization proves less effective. For operational platforms like TalentBridge Connect, the hybrid algorithm represents the optimal balance, though periodic verification using genetic algorithms could ensure continued robustness.
...
Based on the experimental findings, we recommend the hybrid algorithm as the primary solution approach for operational deployment in platforms like TalentBridge Connect. Its combination of kernelization preprocessing with adaptive algorithm selection achieves an optimal balance between solution quality and computational efficiency. The algorithm's ability to solve instances with 150 freelancers in under 0.001 seconds while maintaining optimality makes it suitable for real-time applications with stringent performance requirements.
